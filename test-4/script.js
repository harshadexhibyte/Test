...
function stringMerge(s1, s2) {
  if (s1 == "") {
    ...
    return s2[0] + stringMerge(s1, s2.slice(1));
  }
  return s1[0] + stringMerge(s1.slice(1), s2);
}

function checkOccurrence(s) {
  for (let i = 0; i < s.length; i++) {
    if (s.indexOf(s[i]) !== s.lastIndexOf(s[i])) {
      return false; //dublicat
    }
  }
  return true;
}
...
if (checkOccurrence(string1) && checkOccurrence(string2)) {
  tower = stringMerge(string1, string2);
  console.log(tower);
} else {
...
console.log(`Call to test  ${endTime - startTime} milliseconds`);
